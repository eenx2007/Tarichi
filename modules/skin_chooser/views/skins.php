...
<script type="text/javascript">
	$(document).ready(function(){
			...
			$('.activate').live('click',function(){
					...
					$('#loader_image').show();
					$.post('<?=site_url('ajax_things/update_skin');?>',{ site_skin : s_skin },function(data){
	...
	<div class="formbox penuh">
		...
		foreach($skin_list as $skins)
		{
			...
			?>
        <tr class="<?=$odev;?>">
 	        <td><?=$readinfo[0];?> <img src="<?=base_url();?>blueprint/images/preview.png" style="float:right;cursor:pointer;" title="preview" class="previewbtn">
            	<div class="preview" style="position:absolute;width:200px;height:127px;border:2px solid #666;padding:5px;background:#FFF;display:none;">
                	<img src="<?=base_url();?>skins/<?=$skins;?>/_info/preview.png" />
                </div>
            </td>
            <td><?=$readinfo[1];?></td>
            <td><?=$readinfo[2];?></td>
            <td>
				...
				?>
                	<span class="not_active"><a href="javascript:void(0);" title="<?=$skins;?>" class="activate">Activate</a></span>
                <? } ?>
            </td>
		</tr>       
        <? } } ?>
