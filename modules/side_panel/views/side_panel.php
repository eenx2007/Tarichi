...
<script type="text/javascript">
	$(document).ready(function(){
				...
				$('#btn_add_new').click(function(){
					...
					$('#dialog_add_new').dialog('option', 'buttons', {
									...
									"Add" : function() {
										...
										$.post('<?php echo site_url('side_panel/save_side_panel');?>',{ side_panel_type : s_panel_type, side_panel_title : s_panel_title, side_panel_status : s_panel_status }, function(data){
											$('#loader_thing').fadeOut();
											$('#dialog_add_new').dialog('close');
											$('#side_panel_title').val('');
											if(data == 0)
												$('#side_panel_list').load('<?php echo site_url('side_panel/side_panel_list/0');?>');
											else
												$('#side_panel_list2').load('<?php echo site_url('side_panel/side_panel_list/1');?>');
																																											  });
									}
										});		
				...
				$('.deletebtn').live('click',function(){
							...
							$('#loader_thing').show();
							$.post('<?php echo site_url('side_panel/delete_side_panel');?>',{ side_panel_id : s_p_id }, function(data){
									    if(s_p_status==0)
											$('#side_panel_list').load('<?php echo site_url('side_panel/side_panel_list/0');?>');
										else
											$('#side_panel_list2').load('<?php echo site_url('side_panel/side_panel_list/1');?>');
