...
<div id="left_content" class="span-16">
    ...
    <div class="the_post span-16">
        ...
        <div class="post_info span-16">
            <span class="post_date"><?=$sp_date;?></span> <span class="post_tags">Tags: <?=$sp_tags;?></span><a href="javascript:void(0);" style="text-decoration:none;" class="go_to_comment" title="Lihat Komentar"><span class="post_comment"><?=$sp_totalcomment;?> Comments</span></a>
        </div>
        <?=$sp_content;?>
    
     	<!--- comments here -->
        <div class="total_comment span-8">
            <?=$sp_totalcomment;?> Komentar
        </div>
		...
		<? $i=0;
        	...
        	<div class="span-16 comment_box" id="comment_<?=$the_comments['id'];?>">
                <div class="comment_title span-16" >
                    <div class="span-13">Dari : <?=$the_comments['sender'];?> | <span class="small"> <?=$the_comments['date'];?></span> </div>
                    <div class="comment_reply span-3 last"><a href="javascript:void(0);" class="balas_comment" title="<?=$the_comments['id'];?>">Balas &crarr;</a></div>
                </div>
                <div class="span-2" style="padding-top:10px;">
               
                    <img src="<?=$this->template_model->get_gravatar($the_comments['email']);?>" height="60" width="60" style="padding:3px;background:#666;border-radius:3px;" />
                </div>
                <div class="comment_content span-14 last"><?=$the_comments['content'];?>
					<? if($the_comments['subcomment']<>0)
                    { ?>
                        <? foreach($the_comments['child'] as $childcomment): ?>
                           <div class="reply_comment span-14" id="comment_<?=$childcomment['id'];?>"> 
						   		<div class="span-12">
                                	<span>
                                    	balasan oleh <?=$childcomment['sender'];?> @ <?=$childcomment['date'];?>
                                        <hr />
										<?=$childcomment['content']; ?>
                                    </span>
                                </div>
                                <div class="span-2 last" style="text-align:right">
                                	 <img src="<?=$this->template_model->get_gravatar($childcomment['email']);?>" height="30" width="30" style="margin:10px;padding:1px;background:#666;border-radius:3px;" />
                                </div>
                           </div>
                        <? endforeach; ?>
                    <? } ?>
			...
			$(document).ready(function(){
				...
				$('form').submit(function(){
					...
					else
					{
						...
						var jqxhr = $.getJSON("<?=site_url('skin_engine/read_post/refresh_captcha');?>", function(data) {
							
  							$('#captchatexthid').val(data.word);
							$('#capimage').attr('src','<?=base_url();?>captcha/'+data.time+'.jpg');
							$('#captchatext').val('');
						});
						return false;
					}
        ...
        <div class="the_comment span-16" id="comment_box">
			...
			<?=form_open('skin_engine/read_post/send_comment/'.$sp_id);?>
                ...
                <div class="span-5">
                    ...
                    <label>Tuliskan</label><img src="<?=base_url();?>captcha/<?=$captcha['time'];?>.jpg" id="capimage" /><br />
                    <input type="hidden" name="captchatexthid" id="captchatexthid" value="<?=$captcha['word'];?>" />
                    <input type="text" name="captchatext" id="captchatext" />
                    <input type="hidden" name="uristring" id="uristring" value="<?=$this->uri->uri_string();?>" />
